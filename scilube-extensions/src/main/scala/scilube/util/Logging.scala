...
trait Logging {
    ...
    protected def debug(msg: => String, e: => Throwable) {if (log.isDebugEnabled) {log.debug(msg, e)}}

    protected def info(msg: => String) {if (log.isInfoEnabled) {log.info(msg)}}

    protected def info(msg: => String, e: => Throwable) {if (log.isInfoEnabled) {log.info(msg, e)}}
    ...
    protected def error(msg: => String, e: => Throwable) {if (log.isErrorEnabled) {log.error(msg, e)}}

    protected def trace(msg: => String) {if (log.isTraceEnabled) {log.trace(msg)}}

    protected def trace(msg: => String, e: => Throwable) {if (log.isTraceEnabled) {log.trace(msg, e)}}
}
