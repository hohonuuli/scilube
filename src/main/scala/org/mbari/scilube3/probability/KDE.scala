package org.mbari.scilube3.probability
...
object KDE {
  ...
  def apply(
    ...
    val xmesh = Range.BigDecimal(0d, R, dx).map(_ + min).map(_.toDouble).toArray
  ...
  protected[probability] def dct1d(data: Array[Double]): Array[Double] = {
    ...
    val weight = Complex[Double](1, 0) +: (for (j <- 1 until nrows) yield {
      2d * ComplexUtil.exp(Complex[Double](0, -j) * Pi / (2 * nrows))
    })
